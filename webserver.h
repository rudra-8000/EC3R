...
// Inline HTML for the file manager SPA
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  #space{font-size:.85rem;color:#555;margin-bottom:14px}
  .bar-bg{height:10px;background:#ddd;border-radius:5px;margin-top:4px}
  ...
  #upload-area{background:#fff;border:2px dashed #4a90e2;border-radius:8px;padding:24px;text-align:center;cursor:pointer;transition:background .2s}
  #upload-area.drag{background:#e8f0fe}
  #file-input{display:none}
  #progress{display:none;margin-top:12px}
  progress{width:100%;height:16px;border-radius:4px}
  #status{margin-top:8px;font-size:.85rem;color:#555}
  button.reboot{background:#e67e22;color:#fff;border:none;padding:8px 18px;border-radius:6px;cursor:pointer;margin-top:12px;font-size:.9rem}
...
function loadSpace(){
  fetch('/space').then(r=>r.json()).then(d=>{
  ...
  }).catch(()=>document.getElementById('space').textContent='Could not load storage info');
}

function loadBooks(){
  fetch('/list').then(r=>r.json()).then(books=>{
    const tb=document.getElementById('book-list');
    if(!books.length){tb.innerHTML='<tr><td colspan="3" class="empty">No books uploaded yet.</td></tr>';return;}
    tb.innerHTML=books.map(b=>
      `<tr><td>${b.name}</td><td>${fmt(b.size)}</td><td>
...
function del(name){
  ...
  fetch('/delete?f='+encodeURIComponent(name)).then(()=>{loadBooks();loadSpace();});
}

function reboot(){
  if(!confirm('Reboot device?'))return;
  fetch('/reboot').then(()=>document.getElementById('status').textContent='Rebooting...');
}
...
function uploadFiles(files){
  ...
  function next(){
    if(i>=files.length){loadBooks();loadSpace();status.textContent='Upload complete!';return;}
    const f=files[i++];
    ...
    xhr.onload=()=>{prog.value=0;next();};
    xhr.onerror=()=>{status.textContent='Error uploading '+f.name;prog.value=0;next();};
    xhr.send(fd);
  }
  next();
}
...
ua.addEventListener('drop',e=>{e.preventDefault();ua.classList.remove('drag');uploadFiles(e.dataTransfer.files);});
...
inline void webServerInit() {
  // Serve inline SPA — no LittleFS file required
