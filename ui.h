 ...
 *   COLS = (400-4)/6 = 66 chars/line
 *   TEXT_ROWS = (300 - 9 - 8 - 2) / 8 = 35 lines
 */
...
// Landscape dims after setRotation(1): physical 400x300
#define DISP_W    400
#define DISP_H    300
#define FONT_W    6
#define FONT_H    8
#define MARGIN_X  2
#define MARGIN_Y  9       // first text baseline (below title bar)
#define LINE_H    (FONT_H + 1)   // 9px per row
#define COLS      ((DISP_W - 2*MARGIN_X) / FONT_W)          // 66
#define TEXT_ROWS ((DISP_H - MARGIN_Y - LINE_H - 2) / LINE_H) // 32 text rows + 1 status
#define TITLEBAR_H (FONT_H + 3)  // 11px
#define MENU_VIS  ((DISP_H - TITLEBAR_H - 2) / LINE_H)      // ~32 — cap at 20
...
static int renderTextPage(Disp& d, const char* buf, int bufLen,
  ...
  char t[30]; strncpy(t, title, 29); t[29] = '\0';
  char status[72];
  snprintf(status, sizeof(status), "%-29s p%d", t, pageNum + 1);
  d.setCursor(MARGIN_X, DISP_H - LINE_H);
  d.print(status);
  ...
  while (i < bufLen && row < TEXT_ROWS) {
    ...
    if (wLen > COLS) {
      int r = COLS - col;
      d.setCursor(MARGIN_X + col * FONT_W, MARGIN_Y + row * LINE_H);
...
static void renderImagePage(Disp& d, const uint8_t* jpgBuf, size_t jpgLen,
  ...
  _jpgX0 = (DISP_W - sw) / 2;
  _jpgY0 = (DISP_H - LINE_H - sh) / 2;
  TJpgDec.setCallback(_jpgCb);
  TJpgDec.setJpgScale(scale);
  TJpgDec.drawJpg(0, 0, jpgBuf, jpgLen);
  d.setFont(nullptr); d.setTextSize(1); d.setTextColor(GxEPD_BLACK);
  d.setCursor(MARGIN_X, DISP_H - LINE_H); d.print(title);
}
...
static void sesRender(Disp& d) {
  ...
  if (!_ses.hasText) {
    ...
    do {
      ...
      d.setCursor(MARGIN_X, DISP_H/2+LINE_H+2); d.print("Press NEXT to continue");
    } while(d.nextPage());
    return;
  }
  int nextOff=0;
  d.setFullWindow(); d.firstPage();
  do {
    nextOff=renderTextPage(d,_ses.textBuf,_ses.textLen,(int)_ses.byteOff,_ses.pageNum,_ses.title);
  } while(d.nextPage());
  _ses._nextOff=nextOff;
}

// ── Helper: advance chapter ────────────────────────────────────────────────────
...
void uiDrawMenu(Disp& d, const AppState& s) {
  ...
  do {
    ...
    d.setCursor(MARGIN_X, 2);
    d.print("eReader   NEXT/PREV=scroll  SEL=open  BCK=wifi");
    d.setTextColor(GxEPD_BLACK);

    if (s.bookCount == 0) {
      int y = TITLEBAR_H + LINE_H * 2;
      d.setCursor(MARGIN_X, y); y+=LINE_H; d.print("No books found.");
      d.setCursor(MARGIN_X, y); y+=LINE_H; d.print("Connect to WiFi AP and upload EPUBs:");
      d.setCursor(MARGIN_X, y); y+=LINE_H; d.print("  SSID: eReader   Pass: readbooks");
      d.setCursor(MARGIN_X, y);             d.print("  http://192.168.4.1");
    } else {
      int maxVis = (DISP_H - TITLEBAR_H - 2) / LINE_H;
      for (int i = 0; i < maxVis && (s.menuScroll + i) < s.bookCount; i++) {
        int bi = s.menuScroll + i;
        // Top of this row in pixels
        int rowY = TITLEBAR_H + 1 + i * LINE_H;

        if (bi == s.menuIndex) {
          // Highlight bar exactly covering this row
        ...
        // Text baseline = rowY + FONT_H - 1
        d.setCursor(MARGIN_X + (s.openBookIdx == bi ? 0 : FONT_W),
                    rowY + FONT_H - 1);

        const char* fname = s.books[bi];
        const char* slash = strrchr(fname, '/');
        if (slash) fname = slash + 1;
        char dname[50]; strncpy(dname, fname, 49); dname[49]='\0';
      ...
      if (s.menuScroll > 0) {
        d.setTextColor(GxEPD_BLACK);
        d.setCursor(DISP_W - FONT_W * 3, TITLEBAR_H + FONT_H);
        d.print("[^]");
      }
      if (s.menuScroll + maxVis < s.bookCount) {
        d.setTextColor(GxEPD_BLACK);
        d.setCursor(DISP_W - FONT_W * 3, DISP_H - LINE_H);
        d.print("[v]");
      }
    }
  } while (d.nextPage());
...
void uiDrawWifiInfo(Disp& d) {
  ...
  do {
    ...
    d.setTextColor(GxEPD_BLACK);
    int y = TITLEBAR_H + LINE_H * 2;
    d.setCursor(MARGIN_X, y); y+=LINE_H+2; d.print("SSID : eReader");
    d.setCursor(MARGIN_X, y); y+=LINE_H+2; d.print("Pass : readbooks");
    d.setCursor(MARGIN_X, y); y+=LINE_H+4; d.print("URL  : http://192.168.4.1");
    d.setCursor(MARGIN_X, y); y+=LINE_H+2; d.print("Upload .epub then use Reboot button");
...
void uiOpenBook(Disp& d, AppState& s) {
  ...
  // s.books[bi] is now the bare filename; prepend /books/
  ...
  if (!epubOpen(_ses.epubPath, _ses.info)) {
    ...
    do {
      ...
      d.setCursor(MARGIN_X, DISP_H/2+LINE_H+2);  d.print("[BACK] to menu");
    } while(d.nextPage());
...
void handleMenuInput(Disp& d, AppState& s, ButtonEvent ev) {
  ...
  if (ev == BTN_EVT_NEXT) {
    if (s.menuIndex < s.bookCount - 1) {
      s.menuIndex++;
      if (s.menuIndex >= s.menuScroll + (DISP_H-TITLEBAR_H-2)/LINE_H) s.menuScroll++;
      redraw = true;
    }
    ...
    s.mode = MODE_READING;
    uiOpenBook(d, s); return;
  } else if (ev == BTN_EVT_BACK) {
    s.mode = MODE_WIFI_INFO;
    uiDrawWifiInfo(d); return;
  }
...
void handleReadingInput(Disp& d, AppState& s, ButtonEvent ev) {
  ...
  if (ev == BTN_EVT_NEXT) {
    ...
    if (needNextChapter) {
      ...
      sesCachePush(newOff); _ses.byteOff=newOff; _ses.pageNum++;
    }
    ...
    if (_ses.pageNum > 0 && _ses.cCount > 1) {
      ...
      if (_ses.hasText && _ses.textBuf) {
        ...
        while(true){
          ...
          sesCachePush(next); _ses.pageNum++; off=next;
        }
        _ses.byteOff=off;
      }
    }
