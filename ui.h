...
static void sesFree() {
  if (_ses.textBuf) { bookFree(_ses.textBuf); _ses.textBuf = nullptr; }
  if (_ses.imgBuf)  { bookFree(_ses.imgBuf);  _ses.imgBuf  = nullptr; }
  _ses.textLen = 0;
  _ses.imgLen = 0;
  _ses.hasText = false;
  _ses.isImg = false;
...
static void renderImagePage(Disp& d, const uint8_t* buf, size_t len, const char* title) {
  ...
  _jpgX0 = (DISP_W - iw / scale) / 2;
  _jpgY0 = (DISP_H - LINE_H - ih / scale) / 2;
...
static void _jobLoadChapter(void*) {
  ...
  char* html = nullptr;
  size_t hLen = _epubZipRead.extractToHeap(href, &html, CHAPTER_HTML_MAX, bookAlloc);
  _epubZipRead.close();

  if (!hLen || !html) {
    if (html) bookFree(html);
    return;
  }
  ...
  _ses.textLen = stripHtml(html, (int)hLen, _ses.textBuf, CHAPTER_TEXT_MAX, imgSrc, MAX_HREF_LEN);
  bookFree(html);
  ...
  *q = '\0';
  ...
  if (strlen(imgSrc) > 0 && _ses.textLen < 80) {
    ...
    if (_epubZipImg.open(_ses.epubPath)) {
      char* ib = nullptr;
      size_t il = _epubZipImg.extractToHeap(ipath, &ib, IMAGE_MAX_BYTES, bookAlloc);
      _epubZipImg.close();
...
static bool sesLoadChapter(int ci) {
  _ses.chIdx = ci;
  return runPinnedJob(_jobLoadChapter, nullptr, WORKER_CORE, WORKER_STACK_BYTES) && _ses.loadOk;
}
...
static void sesRender(Disp& d) {
  if (_ses.isImg) {
    ...
    d.firstPage();
    do {
      renderImagePage(d, _ses.imgBuf, _ses.imgLen, _ses.title);
    } while (d.nextPage());
    return;
  }
  ...
  if (!_ses.hasText) {
    ...
    do {
      ...
      d.setCursor(MARGIN_X, DISP_H / 2 + LINE_H);
...
void uiOpenBook(Disp& d, AppState& s) {
  ...
  if (!sesLoadChapter(_ses.chIdx)) {
    ...
    do {
      ...
      d.setTextColor(GxEPD_BLACK);
      d.setCursor(MARGIN_X, DISP_H / 2);
      d.print("Failed to load chapter.");
      d.setCursor(MARGIN_X, DISP_H / 2 + LINE_H);
      d.print("[BACK] to menu");
    } while (d.nextPage());
    s.mode = MODE_MENU;
