...
extern bool runPinnedJob(void (*fn)(void*), void* arg, BaseType_t core, uint32_t stackBytes);
...
#define PAGE_CACHE  16

#define CHAPTER_HTML_MAX   (256 * 1024)
#define CHAPTER_TEXT_MAX   (256 * 1024)
#define IMAGE_MAX_BYTES    (256 * 1024)
...
static void* bookAlloc(size_t n) {
  void* p = nullptr;
  if (psramFound())
...
static bool _jpgCb(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t* bmp) {
  ...
  for (int by = 0; by < h; by++)
    for (int bx = 0; bx < w; bx++) {
      ...
      int16_t dx = _jpgX0+x+bx, dy = _jpgY0+y+by;
      if (dx>=0&&dx<DISP_W&&dy>=0&&dy<DISP_H-LINE_H)
        _jpgDisp->drawPixel(dx, dy, lum<128?GxEPD_BLACK:GxEPD_WHITE);
    }
  return true;
}
...
// ---- Worker jobs -----------------------------------------------------
static void _jobEpubOpen(void*) {
  _ses.loadOk = epubOpen(_ses.epubPath, _ses.info);
}

static void _jobLoadChapter(void*) {
  sesFree();
  _ses.loadOk = false;
  if (!_epubZipRead.open(_ses.epubPath)) return;

  const char* href = _ses.info.chapterHrefs[_ses.chIdx];
  char* html = nullptr;
  size_t hLen = _epubZipRead.extractToHeap(href, &html, CHAPTER_HTML_MAX,
                                            (ZipAllocFn)bookAlloc);
  _epubZipRead.close();
  if (!hLen || !html) { if (html) free(html); return; }

  _ses.textBuf = (char*)bookAlloc(CHAPTER_TEXT_MAX);
  if (!_ses.textBuf) { free(html); return; }

  char imgSrc[MAX_HREF_LEN] = {};
  _ses.textLen = stripHtml(html, (int)hLen, _ses.textBuf, CHAPTER_TEXT_MAX,
                            imgSrc, MAX_HREF_LEN);
  free(html);
  ...
  *q = '\0';
  _ses.textLen = q - _ses.textBuf;
  _ses.hasText = (_ses.textLen > 4);

  // image-only chapter
  if (strlen(imgSrc) > 0 && _ses.textLen < 80) {
    char ipath[MAX_PATH_LEN + MAX_HREF_LEN];
    snprintf(ipath, sizeof(ipath), "%s%s", _ses.info.opfBase, imgSrc);
    if (_epubZipImg.open(_ses.epubPath)) {
      char* ib = nullptr;
      size_t il = _epubZipImg.extractToHeap(ipath, &ib, IMAGE_MAX_BYTES,
                                             (ZipAllocFn)bookAlloc);
      _epubZipImg.close();
      if (il && ib) {
        _ses.imgBuf = (uint8_t*)ib;
        _ses.imgLen = il;
        _ses.isImg  = true;
        _ses.hasText = false;
      }
...
static bool sesLoadChapter(int ci) {
  _ses.chIdx = ci;
  return runPinnedJob(_jobLoadChapter, nullptr, WORKER_CORE, WORKER_STACK_BYTES)
         && _ses.loadOk;
}
...
// ---- Full-update render (book pages + first draw) --------------------
...
static bool sesAdvanceChapter(Disp& d) {
  ...
  if (!sesLoadChapter(_ses.chIdx)) return false;
...
// =========================================================================
// Menu drawing
// uiDrawMenu       = full refresh (first draw, after book close)
// uiUpdateMenuRow  = partial refresh (cursor movement only)
// =========================================================================

// Draws one menu row into the display buffer (call inside firstPage loop)
static void _drawMenuRow(Disp& d, const AppState& s, int i) {
  ...
  if (bi == s.menuIndex) {
    ...
    d.fillRect(0, rowY, DISP_W, LINE_H, GxEPD_WHITE);
...
// Full refresh of the whole menu (on first draw or after scroll)
  ...
  do {
    ...
    if (s.bookCount == 0) {
      ...
      for (int i=0; i<MENU_ROWS && (s.menuScroll+i)<s.bookCount; i++)
        _drawMenuRow(d, s, i);
      if (s.menuScroll > 0) {
        d.setCursor(DISP_W-FONT_W*4, TITLEBAR_H+1); d.print(" [^]");
      }
      if (s.menuScroll+MENU_ROWS < s.bookCount) {
        d.setCursor(DISP_W-FONT_W*4, DISP_H-LINE_H+1); d.print(" [v]");
      }
    }
  } while(d.nextPage());
}

// Partial refresh: only repaint the two rows that changed (old and new cursor)
// prevIndex = menuIndex before the button press
void uiMenuMoveCursor(Disp& d, const AppState& s, int prevIndex) {
  d.setFont(nullptr); d.setTextSize(1);

  auto refreshRow = [&](int bi) {
    int i = bi - s.menuScroll;  // row within visible window
    if (i < 0 || i >= MENU_ROWS) return;
    int rowY = TITLEBAR_H + 1 + i * LINE_H;
    d.setPartialWindow(0, rowY, DISP_W, LINE_H);
    d.firstPage();
    do { _drawMenuRow(d, s, i); } while(d.nextPage());
  };

  refreshRow(prevIndex);   // deselect old row
  refreshRow(s.menuIndex); // select new row
}
...
void uiOpenBook(Disp& d, AppState& s) {
  ...
  // Full refresh for the "Opening..." splash
  d.setFullWindow(); d.firstPage();
  do {
  ...
  if (!runPinnedJob(_jobEpubOpen, nullptr, WORKER_CORE, WORKER_STACK_BYTES) || !_ses.loadOk) {
    d.setFullWindow(); d.firstPage();
    do {
      d.fillScreen(GxEPD_WHITE); d.setFont(nullptr); d.setTextSize(1); d.setTextColor(GxEPD_BLACK);
  ...
  if (!sesLoadChapter(_ses.chIdx)) {
    d.setFullWindow(); d.firstPage();
    do {
      d.fillScreen(GxEPD_WHITE); d.setFont(nullptr); d.setTextSize(1); d.setTextColor(GxEPD_BLACK);
      d.setCursor(MARGIN_X,DISP_H/2);       d.print("Failed to load chapter.");
      d.setCursor(MARGIN_X,DISP_H/2+LINE_H);d.print("[BACK] to menu");
    } while(d.nextPage());
    s.mode = MODE_MENU; return;
  }
  ...
  sesRender(d); // full update for first page of book
}
...
void handleMenuInput(Disp& d, AppState& s, ButtonEvent ev) {
  ...
  if (ev == BTN_EVT_NEXT) {
    if (s.menuIndex < s.bookCount - 1) {
      int prev = s.menuIndex;
      s.menuIndex++;
      if (s.menuIndex >= s.menuScroll + MENU_ROWS) {
        s.menuScroll++;           // scroll: full refresh needed
        uiDrawMenu(d, s);
      } else {
        uiMenuMoveCursor(d, s, prev);  // in-window: partial refresh
      }
    }
...
void handleWifiInfoInput(Disp& d, AppState& s, ButtonEvent ev) {
  if (ev == BTN_EVT_BACK || ev == BTN_EVT_SELECT) {
    s.mode = MODE_MENU; uiDrawMenu(d, s);
  }
}
...
void handleReadingInput(Disp& d, AppState& s, ButtonEvent ev) {
  ...
  if (ev == BTN_EVT_NEXT) {
    ...
    if (_ses.pageNum > 0 && _ses.cCount > 1) {
      _ses.byteOff = sesCachePop(); _ses.pageNum--;
